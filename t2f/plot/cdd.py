...
def draw_cd_diagram(df_perf, out_filename, alpha=0.05, title=None, labels=False):
    """
    Draws the critical difference diagram given the list of pairwise classifiers that are
    significant or not
    """
    assert np.all(
        [x in df_perf.columns for x in ['classifier_name', 'dataset_name', 'accuracy']]), 'Missing mandatory columns'
