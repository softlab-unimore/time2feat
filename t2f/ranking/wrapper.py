...
class Ranker(object):
    ...
    def ranking(self, df: pd.DataFrame, y: list) -> list:
        ...
        for i, ranker in tqdm(enumerate(self.rankers)):
            # Get the rank from each ranker
            rank = ranker(df.copy(), np.array(y).copy())
            ...
            # Drop duplicated features/index
            rank = rank[~rank.index.duplicated()]
            # Name the rank series for identification
            rank.name = self.ranking_type[i]
